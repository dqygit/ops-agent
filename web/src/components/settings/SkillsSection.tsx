...
function SkillCard({ skill }: { skill: SkillPackage }) {
  const statusLabel = skill.valid ? 'Valid' : 'Invalid'
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex min-w-0 flex-col gap-1.5">
            ...
            <p className="text-[11px] text-ops-muted leading-5">{skill.description || 'No description provided.'}</p>
          </div>
        </div>

        <dl className="grid gap-3 sm:grid-cols-2">
          <div className="flex flex-col gap-1 rounded-xl border border-ops-border/10 bg-ops-deep/30 px-4 py-3">
            <dt className="text-[10px] font-bold tracking-widest text-ops-muted/70">PATH</dt>
            <dd className="font-mono text-[11px] text-ops-text break-all">{skill.path}</dd>
          </div>
          <div className="flex flex-col gap-1 rounded-xl border border-ops-border/10 bg-ops-deep/30 px-4 py-3">
            <dt className="text-[10px] font-bold tracking-widest text-ops-muted/70">UPDATED</dt>
            <dd className="text-[11px] text-ops-text">{formatTimestamp(skill.updatedAt)}</dd>
          </div>
          <div className="flex flex-col gap-1 rounded-xl border border-ops-border/10 bg-ops-deep/30 px-4 py-3">
            <dt className="text-[10px] font-bold tracking-widest text-ops-muted/70">BODY SIZE</dt>
            <dd className="text-[11px] text-ops-text">{formatBodySize(skill.bodySize)}</dd>
          </div>
          <div className="flex flex-col gap-1 rounded-xl border border-ops-border/10 bg-ops-deep/30 px-4 py-3">
            <dt className="text-[10px] font-bold tracking-widest text-ops-muted/70">STATUS</dt>
        ...
        {!skill.valid && skill.error ? (
          <div className="rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3">
            <div className="text-[10px] font-bold tracking-widest text-red-300">LOAD ERROR</div>
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between pb-4 border-b border-ops-border/20">
        <div>
          <h4 className="text-[14px] font-bold tracking-[0.15em] text-ops-text">Skill Packages</h4>
          <p className="text-[10px] font-medium text-ops-muted mt-1 tracking-wider opacity-60">Loaded from /api/skills. Valid and invalid packages are both listed for inspection.</p>
        </div>
        <div className="text-right text-[10px] font-medium tracking-wider text-ops-muted opacity-70">
          <div>Total: {skills.length}</div>
          <div>Valid: {validCount} / Invalid: {invalidCount}</div>
        </div>
      </div>

      {loading ? <div className="flex items-center justify-center h-40 text-ops-muted text-sm">Loading skills...</div> : null}
      ...
      {!loading && !error && skills.length === 0 ? (
        <div className="text-center py-10 text-ops-muted text-sm bg-ops-panel/20 rounded-lg border border-ops-border/10 border-dashed">No skill packages found.</div>
      ) : null}
