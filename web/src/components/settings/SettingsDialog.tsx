...
export function SettingsDialog({ initialGroups, selectedModel, onSelectedModelChange, onGroupsChange, onModelOptionsChange, onClose }: SettingsDialogProps) {
  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <section className="asset-modal settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title">
        <div className="settings-header">
          <div>
            <h3 id="settings-title" className="modal-title">设置</h3>
            <p className="modal-description">管理分组和模型配置。</p>
          </div>
          <button type="button" className="button" onClick={onClose}>关闭</button>
        </div>
        <div className="settings-layout">
          <nav className="settings-nav" aria-label="设置导航">
            <button type="button" className={`settings-nav-button ${activeSection === 'groups' ? 'settings-nav-button-active' : ''}`} onClick={() => setActiveSection('groups')}>分组</button>
            <button type="button" className={`settings-nav-button ${activeSection === 'models' ? 'settings-nav-button-active' : ''}`} onClick={() => setActiveSection('models')}>模型</button>
          </nav>
          <div className="settings-content">
            {error ? <div className="settings-error">{error}<button type="button" className="button" onClick={() => void loadSettings()}>重试</button></div> : null}
            {loading ? (
